...
def move_planets(np.ndarray o_positions,
    ...
    # temporary vars
    cdef double * p_resulting_force = <double *> calloc(3, sizeof(double))
    cdef double * v = <double *> calloc(3, sizeof(double))
    cdef double abs_dist_planets = 0.0
    cdef double * accel = <double *> calloc(3, sizeof(double))
    cdef double * pos = <double *> calloc(3, sizeof(double))
    cdef double * speed = <double *> calloc(3, sizeof(double))
    cdef double delta_t_fast = delta_t
    cdef double delta_t_sq_half = ((delta_t_fast ** 2) / 2)
    cdef long num_planets_fast = num_planets

    for i in range(num_planets_fast):

        # clear force and tmp vars
        ...
        for j in range(num_planets_fast):
            ...
            if i != j:
                ...
                abs_dist_planets = sqrt(v[0] ** 2 + v[1] ** 2 + v[2] ** 2)
                #printf("abs_dist_planets: %f\n", abs_dist_planets)
                p_resulting_force[0] += G * ((p_o_masses[i, 0] * p_o_masses[j, 0]) / (abs_dist_planets ** 3)) * v[0]
                p_resulting_force[1] += G * ((p_o_masses[i, 0] * p_o_masses[j, 0]) / (abs_dist_planets ** 3)) * v[1]
                p_resulting_force[2] += G * ((p_o_masses[i, 0] * p_o_masses[j, 0]) / (abs_dist_planets ** 3)) * v[2]
        ...
        #printf("resulting_force: %f, %f, %f\n", p_resulting_force[0], p_resulting_force[1], p_resulting_force[2])
